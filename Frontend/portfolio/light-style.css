...
:root {
	--tt-main-bg-color: #f3f3f3;
	--tt-main-color: #890F0D;
	--tt-ghost-color: #e9e8e5;
	--tt-text-color: #FFF;
	--tt-stroke-color: #111;
}
...
.text-stroke-light {
	-webkit-text-stroke-color: var(--tt-stroke-color);
}

.swiper-pagination-bullet:not(.swiper-pagination-bullet-active) {
	border-color: var(--tt-stroke-color);
}

.tt-pagin-item,
[class*="tt-arrow"] {
	color: var(--tt-stroke-color);
}
...
body:not(.is-mobile):not(.tt-ol-menu-open) .tt-ol-menu-toggle-btn:hover span {
	background-color: var(--tt-stroke-color);
}
...
.tt-form-minimal ::-webkit-input-placeholder {
	color: var(--tt-stroke-color) !important;
}
:-moz-placeholder,
...
body:not(.tt-portfolio-slider-on):not(.tt-portfolio-hover-carousel-on):not(.tt-np-image-on):not(.is-404-page) .tt-btn-light-outline {
	box-shadow: inset 0 0 0 2px var(--tt-stroke-color);
}
